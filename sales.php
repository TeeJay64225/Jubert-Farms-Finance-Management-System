...
function generateInvoiceNo($conn) {
    ...
    $sql = "SELECT MAX(invoice_no) as last_invoice FROM sales WHERE invoice_no LIKE '{$prefix}{$year}{$month}%'";
    $result = $conn->query($sql);
    $row = $result->fetch_assoc();
    
    if ($row['last_invoice']) {
        // Extract the numeric part and increment
        $num = intval(substr($row['last_invoice'], -3)) + 1;
    } else {
        $num = 1;
    }
    
    // Format with leading zeros
    return $prefix . $year . $month . str_pad($num, 3, '0', STR_PAD_LEFT);
}
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $notes = $_POST['notes'];
    $client_id = isset($_POST['client_id']) && !empty($_POST['client_id']) ? $_POST['client_id'] : NULL;
    ...
    if (isset($_POST['sale_id']) && !empty($_POST['sale_id'])) {
        ...
        $invoice_no = $_POST['invoice_no']; // Get invoice_no from form for updates
        ...
        // Generate a new invoice number for new records
        $invoice_no = generateInvoiceNo($conn);
        
        // Add new record
        $sql = "INSERT INTO sales (invoice_no, product_name, quantity, unit_price, amount, sale_date, payment_status, client_id, notes)
                ...
                <div class="row">
                    <div class="col-md-6">
                        <div class="form-group mb-3">
                            ...
                            <input type="text" class="form-control" id="invoice_no" name="invoice_no" value="<?php echo $invoice_no; ?>" readonly>
                        </div>
                    </div>
                </div>
                <?php endif; ?>
                ...
                <div class="row">
                    ...
                    <div class="col-md-6">
                        <div class="form-group mb-3">
                            ...
                            <input type="number" min="1" class="form-control" id="quantity" name="quantity" value="<?php echo $quantity; ?>" onchange="calculateAmount()" required>
                        </div>
                    </div>
                </div>
                ...
                <div class="row">
                    <div class="col-md-6">
                        <div class="form-group mb-3">
                            ...
                            <input type="number" step="0.01" class="form-control" id="unit_price" name="unit_price" value="<?php echo $unit_price; ?>" onchange="calculateAmount()" required>
                        </div>
                    </div>
                    <div class="col-md-6">
                        <div class="form-group mb-3">
                ...
                if ($result->num_rows > 0) {
                    while ($row = $result->fetch_assoc()) {
                        echo "<tr>
                                ...
                                <td>{$row['quantity']}</td>
                                <td>GHS" . number_format($row['unit_price'], 2) . "</td>
                    ...
                    echo "<tr><td colspan='11' class='text-center'>No sales records found</td></tr>";
                }
                ?>
                </tbody>
            </table>
